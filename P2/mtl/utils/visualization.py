...
def compose(
        list_triples, cfg, rgb_mean=None, rgb_stddev=None,
        semseg_color_map=None, semseg_ignore_label=None, depth_range=None):
    ...
    for i, (modality, img, caption) in enumerate(list_triples):
        if modality == MOD_RGB:
            vis.append(tensor_print(
                rgb.clone(), caption if type(caption) is list else
                [str(idx) for idx in caption.tolist()])
            )
        elif modality == MOD_SEMSEG:
            vis.append(tensor_print(
                prepare_semseg(img, semseg_color_map, semseg_ignore_label),
                caption))
