...
import numpy as np
import os
from utils.objects import Visualizer2D

class Visualizer():

    def __init__(self, up='y', azimuth=-90, distance=5):
        ...
        self.view.camera = vispy.scene.cameras.TurntableCamera(up=up, azimuth=azimuth, distance=distance)
    ...
    def update_points(self, points, colors=[1,1,1,1]):
        '''
        Prameters:
    ...
    def update_boxes(self, corners, color=[0,1,0,1]):
        '''
        ...
        self.obj_vis.set_data(corners.reshape(-1,3), connect=connect, width=2, color=color)

    def add_boxes(self, corners, color=[0,1,0,1]):
        for i in range(corners.shape[0]):
            connect = np.concatenate((connect, self.connect + 8 * i), axis=0) \
                if i > 0 else self.connect

        lines = visuals.Line()
        lines.set_data(corners.reshape(-1, 3), connect=connect, width=2, color=color)
        self.view.add(lines)

    def add_points(self, points, color=[1,1,1,1]):
        markers = visuals.Markers()
        markers.set_data(points, size=3, edge_color=color, face_color=color)
